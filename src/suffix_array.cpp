...
void induceSortS(const std::vector<int64_t> &s,
...
void SuffixArray::sa_is(const std::vector<int64_t> &s, std::vector<size_t> &sa, int64_t alphabetSize) {
    ...
    for (size_t i = 1; i < sortedLMSPositions.size(); ++i) {
        ...
        bool differ = false;
        for (size_t d = 0;; ++d) {
            const bool previousLMS = (d > 0) && isLMSPosition(previous + d);
            const bool currentLMS = (d > 0) && isLMSPosition(current + d);

            if (s[previous + d] != s[current + d] || isSType[previous + d] != isSType[current + d] ||
