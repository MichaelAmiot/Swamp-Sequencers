...
void SuffixArray::buildSuffixArray() {
    ...
    for (size_t window = 1; window < _num; window <<= 1) {
       auto compareByIndex = [&](size_t leftIndex, size_t rightIndex) {
            ...
            const int rankA = (leftIndex + window < _num) ? ranks[leftIndex + window] : -1;
            const int rankB = (rightIndex + window < _num) ? ranks[rightIndex + window] : -1;
            return rankA < rankB;
        };

        std::sort(_sa.begin(), _sa.end(), cmp);

        // Re-assign ranks based on the freshly sorted order.
        nextRanks[_sa[0]] = 0;
        for (size_t i = 1; i < _num; ++i) {
            nextRanks[_sa[i]] = nextRanks[_sa[i - 1]] + (cmp(_sa[i - 1], _sa[i]) ? 1 : 0);
        }
